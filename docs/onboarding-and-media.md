...
- Appears right AFTER the name modal, whenever the player had to CHOOSE
  a name (main.ts passes `freshName` into the scene) - two looping
  videos side by side - Walking, then Throwing - each above a pure-CSS
  animated mouse. Walk gets a plain LEFT CLICK loop; throw gets
  CLICK-AND-HOLD (press, long hold, release). Keyframes live in
  `style.css` (`tutClick` / `tutHold`).
- LESSON (2026-07-16): the first version used a browser-global
  localStorage seen-flag set ON SHOW - a page load the owner never
  looked at consumed it, and the pop-up "never appeared". The stored
  name itself is now the persistence: no name stored = first entry =
  name modal + pop-up; nothing else to go stale.
